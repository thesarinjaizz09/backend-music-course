...
const registerUser = async (req: Request, res: Response): Promise<void>  => {
    ...
    try {
        ...
        if (error instanceof ZodError) {
            res.status(400).json({
                success: false,
                errors: error.errors.map(err => err.message)  // Array of validation error messages
            });
...
const loginUser = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (error instanceof ApiError) {
            ...
            res.status(500).json(new ApiResponse(500, null, "An unexpected error occurred while logging in."));
        }
...
const refreshAccessToken = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const storedToken = await db
        ...
        .from(RefreshTokens)
        .where(
          sql`${RefreshTokens.token} = ${refreshToken} AND ${RefreshTokens.expiresAt} > ${new Date()}`
        )
        .limit(1);
      ...
      res.cookie("accessToken", newAccessToken, { ...cookieOptions, maxAge: 15 * 60 * 1000 }); // 15 min
      ...
      return res.status(500).json(new ApiError(500, "An error occurred while refreshing the access token"));
    }
}
